...
for num, indice in enumerate(indices):
    ...
    json_data = {
        "smplx_body_pose": out["smplx_body_pose"].detach().cpu().numpy().reshape(-1, 3).tolist(),
        "smplx_lhand_pose": out["smplx_lhand_pose"].detach().cpu().numpy().reshape(-1, 3).tolist(),
        "smplx_rhand_pose": out["smplx_rhand_pose"].detach().cpu().numpy().reshape(-1, 3).tolist(),
    }

    with open(os.path.join(args.output_folder, f"person_{num}.json"), "w") as f:
